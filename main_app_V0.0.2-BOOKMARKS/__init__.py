...
def get_html(text, number, path_name=""):
    ...
    html_part = '<a href="file://'+path_name+'#page='+str(number)+'"><p><h4>'+text+'&nbsp;</h4></a> PAGE '+str(number)+'</p><hr/>'
...
def convert_HTML_with_TOC(table, name_of_file, title_element, part_num):
    ...
    for name, number in table:
        if(number>=1):
        f1.write(get_html(name, size_toc+number, path_name_pdf))
    f1.close()
...
def pdf_merge(output_path, input_paths, table):
    ...
    #for name, number in table:
        #pdf_merger.addBookmark(name, size_toc+number)
        #pdf_merger.addNamedDestination(name,size_toc+number)
...
#merge PDFS
pdf_merge('FINAL_PREAM_'+title_of_document+'_WITH-TOC.pdf',files, toc_1)
pdf_merge('FINAL_PROPOSED_'+title_of_document+'_WITH-TOC.pdf',files_2, toc_2)
